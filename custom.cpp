...
{
    std::ofstream fileLLS("testLLS.txt");
    Y.resize(6);
    Y[0] = X[3];
    Y[1] = X[4];
    Y[2] = X[5];
    ...
    if (toJulianDate(checkDay)*86400 < t)
    {
        ...
        for(int i = 0; i < 3; i++) Re0[i] = X[i]/X.length()/*ro*/;
        r0[0] = cos(latitude)*cos(longtitude/*Srad*/);
        r0[1] = cos(latitude)*sin(longtitude/*Srad*/);
        r0[2] = sin(latitude);
        /*r0[0] = sin(latitude)*cos(longtitude/*Srad*///);
       // r0[1] = sin(latitude)*sin(longtitude/*Srad*/);
       // r0[2] = cos(latitude);
        Rg = r0*gnomonHeight;
        ...
        file << (t-toJulianDate(checkDay)*86400)/(60*60) << " " <<sqrt(pow(Rsh[0],2)+pow(Rsh[1],2))/*Rsh.length()*/ << " " << daytime << std::endl;
        fileLLS<<"r0[0]: "<<r0[0]<<std::endl;
        fileLLS<<"r0[1]: "<<r0[1]<<std::endl;
        fileLLS<<"r0[2]: "<<r0[2]<<std::endl;
        fileLLS<<"Rg[0]: "<<Rg[0]<<std::endl;
        fileLLS<<"Rg[1]: "<<Rg[1]<<std::endl;
        fileLLS<<"Rg[2]: "<<Rg[2]<<std::endl;
        fileLLS<<"Rsh[0]: "<<Rsh[0]<<std::endl;
        fileLLS<<"Rsh[1]: "<<Rsh[1]<<std::endl;
        fileLLS<<"Rsh[2]: "<<Rsh[2]<<std::endl;
        fileLLS<<"Rg.length(): "<<Rg.length()<<std::endl;
